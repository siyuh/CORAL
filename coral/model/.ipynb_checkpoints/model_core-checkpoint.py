...
class DeconvolutionLayer(nn.Module):
    def __init__(self, input_dim, cell_type_dim, output_dim):
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(input_dim+cell_type_dim, 256),
            nn.ReLU()
        )
        self.fc2 = nn.Sequential(
            nn.Linear(256+cell_type_dim, 64),
            nn.ReLU()
        )
        self.fc3 = nn.Sequential(
            nn.Linear(64+cell_type_dim, output_dim),
        )
    
        
    def forward(self, x, c ):
        
        hidden = self.fc1(torch.cat((x,c),dim=1))
        #hidden = hidden + torch.randn_like(hidden) 
        hidden = self.fc2(torch.cat((hidden,c),dim=1))
        #hidden = hidden + torch.randn_like(hidden) 
        hidden = self.fc3(torch.cat((hidden,c),dim=1))
        #hidden = hidden + torch.randn_like(hidden) 
        return F.softmax(hidden, dim=-1)


class CORAL_model(nn.Module):
    def __init__(self, visium_dim, codex_dim, visium_size, codex_size, cell_type_dim, latent_dim, hidden_channels, v_dim,  high_res_data_dist,low_res_data_dist, eps=1e-10):

        super().__init__()
        
        self.visium_dim = visium_dim
        self.codex_dim = codex_dim
        self.visium_size = visium_size
        self.codex_size = codex_size
        self.cell_type_dim = cell_type_dim
        self.latent_dim = latent_dim
        self.high_res_data_dist = high_res_data_dist
        self.low_res_data_dist = low_res_data_dist
        ...
        # Prior Network for z given v
        self.zi_prior = nn.Sequential(
            nn.Linear(v_dim + cell_type_dim, 128),
            nn.ReLU(),
            nn.Linear(128, latent_dim * 2)
        )
        ...
        self.gat1   = GATConv(latent_dim, hidden_channels, heads=4, concat=True, dropout=0.1)
        self.gat2   = GATConv(hidden_channels * 4, latent_dim, heads=1, concat=False, dropout=0.1)
        ...
        self.v_layer = nn.Sequential(
            nn.Linear(visium_dim + codex_dim+latent_dim + cell_type_dim, v_dim * 2),  
        )
        ...
        self.layer_norm = nn.LayerNorm(hidden_channels * 4)
    ...
    def infer_v(self, x_combined, z, cell_type):
        combined_input = torch.cat((x_combined, z, cell_type), dim=1)
        v_h = self.v_layer(combined_input)
        v_m, v_logvar  = v_h.chunk(2, dim=-1)
    ...
    def forward(self, batch,, device):
        ...
        x, edge_index, cell_type, spot_indices,visium_spot,spatial_coords,center_cell = batch.x, batch.edge_index, batch.cell_type, batch.spot_indices, batch.visium_spot, batch.spatial_coords, batch.center_cell
        ...
        # Infer p_z given v
        p_zi_h = self.zi_prior(torch.cat((v, cell_type), dim=1))
        p_zi_m, p_zi_logvar  = p_zi_h.chunk(2, dim=-1)
        p_zi   = self.reparameterize(p_zi_m, p_zi_logvar)
        
        
        #z = self.gcn(z, edge_index, edge_weight=edge_weights)
        z, attn_weights_1 = self.gat1(z, edge_index, return_attention_weights=True)
        z = self.layer_norm(torch.relu(z))
        ...
        spot_counts = torch.tensor(
            [(spot_indices == spot).sum().item() for spot in visium_spot],
            device=visium_spot.device,
            dtype=torch.float32
        )
        ...
        for j, spot_ in enumerate(visium_spot):
            ...
            aggregated_visium[j] = torch.sum(visium_rate[spot_indices == spot_], dim=0) / (spot_counts[j] + 1e-6)

        
        
        return {
            'pxi_rate':visium_rate[center_cell],
            'px_rate_aggregated': aggregated_visium,
            'px_r': self.px_r,
            
            'py_rate': codex_rate[center_cell],
            'py_r': self.py_r,
            
            'z_mu': z_mu[center_cell],
            'z_logvar': z_logvar[center_cell],
            'z': z,  
            
            'p_zi_m':p_zi_m[center_cell], 
            'p_zi_logvar':p_zi_logvar[center_cell],
            
            'generated_cell_type': generated_cell_type[center_cell],
            
            'px_r_sc':self.px_r_sc,
            'q_xi_m':deconv_x[center_cell],
            'q_xi':q_xi[center_cell],
            
            'v': v[center_cell],
            'v_m': v_m[center_cell],
            'v_logvar': v_logvar[center_cell],
